    ...
    while not sandRest:
        try:
            ...
            # DIAGONAL LEFT
                ...
                if TRACE:
                    print(f"DIAGONAL LEFT: {row + 1}, {col - 1}")
                row += 1
                col -= 1
            # DIAGONAL RIGHT
            elif col + 1 < len(grid[0]) and grid[row + 1][col + 1] == '.':
                if TRACE:
                    print(f"DIAGONAL RIGHT: {row + 1}, {col + 1}")
                row += 1
                col += 1
            # Check bounds, especially if expansion is enabled
            elif col - 1 < 0:
                if TRACE:
                    print(f"Out of scope at {row + 1}, {col - 1}")

                if not expand:
                    return [None, sandStart]

                if TRACE:
                    print(f"Expanding on left by 3 for col {col}")
                ...
                row, col = sandStart[0], sandStart[1]
                if TRACE:
                    print(f"Retrying at START = {row}, {col}")
            ...
            # Check bounds, especially if expansion is enabled
                ...
                if TRACE:
                    print(f"Expanding on right by 3 for col {col}")
        ...
        # Check bounds, cover remaining edge cases
            ...
            sandRest = True
            if TRACE:
                print(f"Out of scope at {row}, {col}")
            return [None, sandStart]
